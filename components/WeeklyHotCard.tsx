...
const WeeklyHotCard: React.FC<WeeklyHotCardProps> = ({ novel, onClick }) => {
    ...
    if (rank === 1) {
      return (
        <div className="relative w-8 h-8 flex items-center justify-center">
          ...
          <span className="relative text-black font-bold text-lg">{rank}</span>
        </div>
      );
    }
    return (
  ...
  return (
    <div onClick={onClick} className="flex items-center space-x-4 py-2 w-full cursor-pointer group">
      ...
      <div className="flex-grow overflow-hidden">
        <h3 className="text-white text-md font-bold truncate group-hover:text-primary transition-colors duration-200" title={novel.title}>
          {novel.title}
        </h3>
        <p className="text-neutral-400 text-sm truncate" title={novel.genre}>
          {novel.genre}
        </p>
      </div>
    </div>
